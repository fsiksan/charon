...
import { circuitBreakerTripped, dailyRealizedPnlSol, canOpenMorePositions, openPositionCount, tradingMode } from '../db/positions.js';
import { SIGNAL_SERVER_URL } from '../config.js';
import {
  menuKeyboard,
  filtersText,
  filtersKeyboard,
...
export async function handleMessage(msg) {
  ...
  if (text.startsWith('/health')) return sendHealth(chatId);
...
export async function sendHealth(chatId) {
  const strat = activeStrategy();
  const agentOn = boolSetting('agent_enabled', true);
  const cbTripped = circuitBreakerTripped();
  const todayPnl = dailyRealizedPnlSol();
  const lossLimit = numSetting('daily_loss_limit_sol', 0.35);
  const cbEnabled = boolSetting('circuit_breaker_enabled', true);
  const openPos = openPositionCount();
  const maxPos = strat.max_open_positions ?? numSetting('max_open_positions', 3);
  const mode = tradingMode();

  // Last signal received (signal_events table — written before any strategy filtering)
  const lastSignal = db.prepare(
    'SELECT at_ms, kind, source FROM signal_events ORDER BY at_ms DESC LIMIT 1'
  ).get();
  const signalAgoMs = lastSignal ? Date.now() - lastSignal.at_ms : null;
  const signalAgoStr = signalAgoMs == null ? 'never'
    : signalAgoMs < 60000 ? `${Math.round(signalAgoMs/1000)}s ago`
    : signalAgoMs < 3600000 ? `${Math.round(signalAgoMs/60000)}m ago`
    : `${(signalAgoMs/3600000).toFixed(1)}h ago`;

  // Last time orchestrator processed a signal (decision_logs — written AFTER strategy filters pass)
  const lastDecision = db.prepare(
    'SELECT at_ms, action, verdict, confidence FROM decision_logs ORDER BY at_ms DESC LIMIT 1'
  ).get();
  const decisionAgoMs = lastDecision ? Date.now() - lastDecision.at_ms : null;
  const decisionAgoStr = decisionAgoMs == null ? 'never'
    : decisionAgoMs < 60000 ? `${Math.round(decisionAgoMs/1000)}s ago`
    : decisionAgoMs < 3600000 ? `${Math.round(decisionAgoMs/60000)}m ago`
    : `${(decisionAgoMs/3600000).toFixed(1)}h ago`;

  // Last 5 decision actions today
  const todayStart = new Date(new Date().toISOString().slice(0, 10) + 'T00:00:00Z').getTime();
  const recentActions = db.prepare(
    'SELECT action, COUNT(*) AS n FROM decision_logs WHERE at_ms >= ? GROUP BY action ORDER BY n DESC'
  ).all(todayStart);

  // Signals received today vs decisions made today
  const todaySignals = db.prepare('SELECT COUNT(*) AS n FROM signal_events WHERE at_ms >= ?').get(todayStart);
  const todayDecisions = db.prepare('SELECT COUNT(*) AS n FROM decision_logs WHERE at_ms >= ?').get(todayStart);

  // Diagnose the bottleneck
  const issues = [];
  if (!agentOn) issues.push('🔴 Agent is DISABLED — turn on via menu');
  if (cbTripped) issues.push(`🔴 Circuit breaker TRIPPED — today loss ${todayPnl.toFixed(3)} SOL ≥ limit ${lossLimit} SOL (resets at UTC midnight)`);
  if (openPos >= maxPos) issues.push(`🟡 Max positions reached (${openPos}/${maxPos}) — waiting for a close`);
  if (signalAgoMs != null && signalAgoMs > 30 * 60 * 1000) issues.push(`🟡 No signals received in ${signalAgoStr} — signal server may be down`);
  if (lastSignal && lastDecision && (lastDecision.at_ms < lastSignal.at_ms - 5 * 60 * 1000) && todaySignals.n > 0 && todayDecisions.n === 0) {
    issues.push('🟡 Signals arriving but none reaching orchestrator — strategy gates (require_fee_claim / min_source_count / token_age) filtering everything');
  }
  if (issues.length === 0 && lastDecision) {
    const recentBuys = recentActions.filter(a => a.action.includes('entry') && !a.action.includes('skip') && !a.action.includes('reject') && !a.action.includes('fail'));
    if (recentBuys.length === 0) issues.push('🟡 Signals reaching LLM but all returning WATCH — market conditions or confidence threshold');
  }

  const diagLine = issues.length ? issues.join('\n') : '🟢 All systems nominal';

  const lines = [
    `🩺 <b>Bot Health Check</b>`,
    '',
    `<b>Agent:</b> ${agentOn ? '✅ enabled' : '🔴 DISABLED'}   <b>Mode:</b> ${mode}   <b>Strategy:</b> ${strat.id}`,
    `<b>Positions:</b> ${openPos}/${maxPos} open`,
    '',
    `<b>Circuit breaker:</b> ${cbEnabled ? (cbTripped ? `🔴 TRIPPED (${todayPnl.toFixed(3)} SOL today)` : `✅ ok (${todayPnl.toFixed(3)} SOL / −${lossLimit} limit)`) : '⚪ disabled'}`,
    '',
    `<b>Signal pipeline today:</b>`,
    `  Signals received : ${todaySignals.n}   (last: ${signalAgoStr})`,
    `  Reached orchestrator: ${todayDecisions.n}   (last: ${decisionAgoStr})`,
    ...(todayDecisions.n === 0 && todaySignals.n > 0 ? ['  ⚠ Gap → strategy gates in serverClient.js dropping all'] : []),
    ...
    `  min_gmgn_fee_sol : ${strat.min_gmgn_total_fee_sol} SOL`,
    `  token_age_max    : ${strat.token_age_max_ms > 0 ? Math.round(strat.token_age_max_ms/60000) + 'm' : 'off'}`,
    `  llm_confidence   : ≥${strat.llm_min_confidence}%`,
    '',
    `<b>Signal source:</b> ${SIGNAL_SERVER_URL || 'standalone WebSocket'}`,
    '',
    `<b>Diagnosis:</b>\n${diagLine}`,
  ];
